...
AGENTS.md grew to 22,768 characters through incremental rule additions. The gateway limit is 20,000 chars. Every agent got a silently truncated version of their instructions for 7 hours.
...
3. Gateway truncated — agents didn't know what they lost
4. A transcript-archiver cron accumulated context without successful compaction
5. Context overflow hit repeatedly (19:00, 19:18, 19:29 UTC)
...
**No automated gate on AGENTS.md file size.** Rules were added individually (each one sensible in isolation) but nobody tracked the cumulative size. This is a textbook "boiling frog" failure.

## The Fix

1. Automated daily cron checks: `wc -c AGENTS.md` with alert at 18K (warning) and 20K (critical)
